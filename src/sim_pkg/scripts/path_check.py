...
class GraphPathPublisher(Node):
    def __init__(self):
        ...
        self.path_waypoints = []      # [(node_id, x, y, yaw), ...]
        self.path_node_ids = []       # ['263', '...', '225']
    ...
    def build_path(self):
        ...
        for (nid, x, y), yaw in zip(coords, yaws):
            self.path_waypoints.append((nid, x, y, yaw))
            pose = PoseStamped()
        ...
        self.get_logger().info(
            f"Built waypoint list with {len(self.path_waypoints)} entries. "
