...
class LanePointProjector(Node):
    def __init__(self):
        ...
        # /odometry/filtered (map -> base_link) 저장용
        self.map_R = None   # 3x3 회전 행렬
        self.map_t = None   # (x, y, z)
    ...
    # ---------------- /odometry/filtered 콜백 (map -> base_link 포즈) ----------------
    ...
    def ekf_callback(self, msg: Odometry):
        # header.frame_id = 'map', child_frame_id = 'base_link' 라고 가정
    ...
    def cloud_callback(self, msg: PointCloud2):
        ...
        if left_lane and right_lane:
            # X 방향으로 binning
            x_min = 0.2
            x_max = 4.0    # 앞 4m 까지 중앙선 사용 (필요시 조정)
        ...
        # ---------------- base_link & map 기준 centerline cloud publish ----------------
        if center_points:
            fields = [
                PointField(name='x', offset=0, datatype=PointField.FLOAT32, count=1),
                PointField(name='y', offset=4, datatype=PointField.FLOAT32, count=1),
            ...
            # 2) map 기준 (/odometry/filtered 이용)
            if self.map_received and self.map_R is not None and self.map_t is not None:
                center_points_map = []
                for xb, yb, zb in center_points:
                    p_base = np.array([xb, yb, zb])
                    # p_map = R_map_base_link * p_base + t_map_base_link
                    p_map = self.map_R @ p_base + self.map_t
                    center_points_map.append(tuple(p_map.tolist()))

                header_center_map = Header()
                header_center_map.stamp = msg.header.stamp
                header_center_map.frame_id = 'map'   # /odometry/filtered frame_id와 동일

                center_cloud_map = pc2.create_cloud(
                    header_center_map, fields, center_points_map
                )
                self.center_cloud_map_pub.publish(center_cloud_map)
